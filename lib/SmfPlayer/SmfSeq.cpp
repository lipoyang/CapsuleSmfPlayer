...
char keyonBuff[SMF_NOTESTATLNG];

void SmfSeqKeyonBuffClear()
{
  for(int i=0; i<SMF_NOTESTATLNG; i++)
  {
    keyonBuff[i] = 0;
  }
}

char * SmfSeqGetKeyonBuff()
{
  return keyonBuff;
}

int  SmfSeqGetKeyonBuffSize()
{
  return SMF_NOTESTATLNG;
}
...
//#LW#          SmfSeqStrcpy( ptrkTbl->SeqTrkName, (char *)ExBuff, SMF_DISPSEQTRK_STRLEN );
        ...
        case  MIDI_STATCH_NOTEON:
          ...
          if((MidiStatus & MIDI_STAT_MASK)==MIDI_STATCH_NOTEON)
          {
            keyonBuff[MidiData1]++;
          }
          Ret = midiOutShortMsg(  (UCHAR)MidiStatus, 
...
//  マスターボリュームセット
//-----------------------------------------------------------------------------
int  SmfSeqSetVolume(int Volume){
  int  Ret;

  Ret = midiOutSetMaterVolume( (UCHAR)Volume );
  if( Ret == MIDI_NG ){
    Ret = SMF_NG;
  }else{
    Ret = SMF_OK;
